...
const recordVideoButt = document.querySelector('.record-video');
const downloadVideoButt = document.querySelector('.download-video');
const metinyazCont = document.querySelector('.metinyaz-cont');
let flag_metin = true;
let flag = true;
const sharefile = document.querySelector('.sharefile');
const filedialog = document.querySelector('.select-file-dialog');




//beyaztahta başlangıç
...
function clearBoard() {
    if (window.confirm('Tahtayı temizlemek ister misiniz? Bu işlem geri alınamaz.')) {

        metinyazCont.innerHTML = '';
        flag_metin = true;
...
continueButt.addEventListener('click', () => {
    ...
    document.querySelector("#myname").innerHTML = `${username} (Siz)`;
...
function handleVideoOffer(offer, sid, cname, micinf, vidinf) {
    ...
    connections[sid].ontrack = function (event) {
        ...
        if (!document.getElementById(sid)) {
            ...
            videoOff.innerHTML = 'Kamera Kapalı'
...
function startRecording() {
    ...
    try {
        ...
        mediaRecorder = new MediaRecorder(mystream, options);//mediatrack dene(burayı vstream olarak değiştirdim) //mystream

        //alert("seçilen tip:" + options.mimeType);

    } catch (e) {
        console.error('MediaRecorder error:', e);
        alert("Odayı oluşturan kişi olmadığınız için kayıt yapamazsınız.");
    }
    mediaRecorder.start();
    //alert("durum:" + mediaRecorder.state);
...
recordVideoButt.addEventListener('click', () => {

    if (downloadVideoButt.style.visibility == "visible") {
        downloadVideoButt.style.visibility = "hidden";
    }

    if (flag == true) {
        startRecording();
        recordVideoButt.innerHTML = `<i id="record_start" class="fas fa-video"></i><span class="tooltiptext">Kayıt Durdur</span>`;
        flag = false;
        ...
        recordVideoButt.innerHTML = `<i class="fas fa-video"></i><span class="tooltiptext">Video Kayıt</span>`;
        flag = true;
        alert("Kayıt tamamlandı!");

        downloadVideoButt.style.visibility="visible";
    }
    
})


downloadVideoButt.addEventListener('click', () => {
    ...
    a.href = url;
    a.download = 'webrtc_record.webm'; //webrtc_record dosya adı. webm ise dosya türü.
...
// beyaztahta metin paylaş başlangıç
function metin() {
    //alert("flag metin:" + flag_metin);
    if (flag_metin == true) {
        metinyazCont.innerHTML += `<textarea class="metinyaz" placeholder="yazılacak metin.." id="metinyaz"></textarea>`;
        let sendmetin = document.querySelector('.metinyaz');
        let metinn = sendmetin.value;
        //alert("metin:" + metinn);
        socket.emit('createmetin', metinn, flag_metin);
        flag_metin = false;
    } else {
        
        sendmetin = document.querySelector('.metinyaz');
        metinn = sendmetin.value;
        //alert("metin:" + metinn);
        socket.emit('createmetin', metinn, flag_metin);
...
socket.on('createmetin', (sendmetin, flag_metin) => {
    //alert("textarea oluşuyor");
    if (flag_metin == true) {
        metinyazCont.innerHTML += '<textarea class="metinyaz" placeholder="yazılacak metin.." id="metinyaz">' + sendmetin + '</textarea>';
    } else {
        metinyazCont.innerHTML = '<textarea class="metinyaz" placeholder="yazılacak metin.." id="metinyaz">' + sendmetin + '</textarea>';
    }
});
// beyaztahta metin paylaş son
