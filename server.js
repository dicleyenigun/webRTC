...
io.on('connect', socket => {
    ...
    socket.on("join room", (roomid, username) => {
        ...
        if (rooms[roomid] && rooms[roomid].length > 0) {
            ...
            socket.to(roomid).emit('message', `${username} odaya katıldı.`, 'Bot', moment().format(
                "h:mm a"
            ));
    ...
    socket.on('createmetin', (sendmetin, flag_metin) => {
        socket.to(socketroom[socket.id]).emit('createmetin', sendmetin, flag_metin);
    });
    ...
    socket.on('disconnect', () => {
        ...
        socket.to(socketroom[socket.id]).emit('message', `${socketname[socket.id]} odadan ayrıldı.`, `Bot`, moment().format(
            "h:mm a"
        ));
